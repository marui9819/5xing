...
4. **文档压缩**：勾选 Word 文档后点击“文档压缩”，可批量选择压缩级别（低/中/高）及输出格式（DOCX/PDF），并生成带有 `_s` 后缀的新文件。
...
- 压缩级别决定图片质量与尺寸上限：低档≈80% 质量且尺寸限制为占位尺寸 × 2.0，中档≈60% 质量且尺寸限制为占位尺寸 × 1.4，高档≈35% 质量且尺寸限制为占位尺寸 × 1.0。
- 图片会根据文档中的显示占位自动调整尺寸，压缩后的宽高不会小于页面上实际占位。
- 已压缩的文档仍可再次选择更高压缩档位，以进一步减小文件体积。
